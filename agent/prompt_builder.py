...
KANBAN_GUIDANCE = (
    ...
    "\n"
    "\n"
    "## HAL (mandatory — HELIos Activity Ledger)\n\n"
    "Every kanban worker MUST leave a findable row in `public.action_ledger` "
    "(Supabase mqtvvacrzmutyrznfwri). Not optional for monitors, retries, or short tasks.\n"
    "1. **OPEN at start** (idempotent per kanban id): "
    "`python3 ~/.hermes/scripts/hal_record.py open --agent \"$HERMES_PROFILE\" "
    "--session \"$HERMES_SESSION_ID\" --kanban \"$HERMES_KANBAN_TASK\" "
    "--job-title \"<task title>\" ` "
    "(add `--credential <CODE>` when SOUL names a binding code). Reuse `action_ledger_id` "
    "from `kanban_show` when present — do not duplicate.\n"
    "2. **CLOSE before `kanban_complete`**: "
    "`python3 ~/.hermes/scripts/hal_record.py close --ledger \"$HAL_LEDGER_ID\" "
    "--session \"$HERMES_SESSION_ID\" --profile \"$HERMES_PROFILE\" --outcome completed`. "
    "Helper stamps tokens/cost from session when possible. Never invent `cost_usd`. "
    "Explicit `$0` only with honest `pricing_source` (`true_zero_*` / NIM free / local Qwen).\n"
    "3. **Assert before done**: "
    "`python3 ~/.hermes/scripts/hal_record.py assert-visible --kanban \"$HERMES_KANBAN_TASK\"` "
    "must exit 0. If it fails, open/close first — do **not** call `kanban_complete`.\n"
    "In-repo path also opens on claim and fail-closes complete via "
    "`hermes_cli/action_ledger.py` + `kanban_db._require_action_ledger_close`.\n"
    "\n"
    "## Do NOT\n"
    "\n"
